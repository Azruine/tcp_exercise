...
char *replace_substring(const char *str, const char *old, const char *new_str) {
    if (!str || !old || !*old) return strdup(str);
...
char *sanitize_error_message(const char *msg) {
    ...
    const char *patterns[] = {"build/src/",      // program build path
...
int compile_submission(const char *source_path, const char *executable_path) {
    ...
    snprintf(command, sizeof(command), "gcc %s -o %s 2>%s", source_path,
             executable_path, COMPILE_ERROR_FILE);
...
int run_test(const char *in_path, const char *expected_out, int *exec_time,
    ...
    if (pid < 0) {
        ...
        if (!WIFEXITED(status) || WEXITSTATUS(status) != 0) {
            fprintf(stderr,
                    "Child process terminated abnormally: status = %d\n",
                    status);
            return -1;
        }
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (entry->d_type == DT_REG) {
            ...
            if (ext && strcmp(ext, ".in") == 0) {
                char in_path[256];
                snprintf(in_path, sizeof(in_path), "%s/%s", IO_DIR,
                         entry->d_name);

                char expected_output[256];
                strncpy(expected_output, entry->d_name,
                ...
                snprintf(expected_path, sizeof(expected_path), "%s/%s", IO_DIR,
                         expected_output);

                int exec_time = 0;
                long mem_usage = 0;
                int test_result = run_test(in_path, expected_path, &exec_time,
                                           &mem_usage, executable_path);
                if (test_result == -1) {
                    overall = -1;
                    FILE *rt_fp = fopen(TEMP_OUTPUT, "r");
                    if (rt_fp) {
                        fread(runtime_error_msg, 1,
                              sizeof(runtime_error_msg) - 1, rt_fp);
                        fclose(rt_fp);
                    }
                    break;
                ...
                {
                    if (exec_time > max_total_time) max_total_time = exec_time;
                    if (mem_usage > max_total_rss) max_total_rss = mem_usage;
                }
            }
        }
    }
